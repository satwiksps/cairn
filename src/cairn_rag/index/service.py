"""High-level, testable orchestration around the pure Cairn core."""
...
from cairn_rag.config import CairnConfig, pending_migration_path
from cairn_rag.content.manifest import CorpusManifest, DocumentManifest, manifest_from_chunks
from cairn_rag.embed import (
    EmbeddingInput,
    create_provider,
    embed_texts,
    embed_with_cache,
...
from cairn_rag.errors import BackendError, ConfigError, ProviderError
from cairn_rag.index.adapters import (
    DocumentState,
    IndexRecord,
    IndexStatus,
    SQLiteIndex,
...
from cairn_rag.index.apply import resolve_identities
from cairn_rag.index.plan import IndexPlan, create_plan
from cairn_rag.index.sources import SourceDocument, discover_sources, load_sources
...
def create_chunker(config: CairnConfig) -> Chunker:
    config.validate()
...
def _protected_state(config: CairnConfig) -> tuple[set[Path], set[Path]]:
    """Return Cairn-managed paths that must never become corpus sources."""
...
def build_target_manifest(
    config: CairnConfig, paths: Sequence[str | Path] = ()
...
def _write_manifest_snapshot(config: CairnConfig, payload: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise BackendError(
            ...
            f"{exc}. Run 'cairn-rag verify' before retrying."
        ) from exc
    finally:
...
def index_status(config: CairnConfig) -> IndexStatus:
    config.validate()
...
def query_index(
    ...
    *,
    ...
    if status.corpus_root is None:
        raise BackendError("Index does not exist or has not been built; run 'cairn-rag index'")
    ...
    if status.model_id != model_id or status.params_hash != params_hash:
        raise ConfigError(
            ...
            "run 'cairn-rag plan' and 'cairn-rag index' before querying"
        )
...
def compact_index(config: CairnConfig, *, before: str | None = None, dry_run: bool = False) -> int:
    config.validate()
    with SQLiteIndex(config.resolve(config.index.database)) as index:
        return index.compact(before=before, dry_run=dry_run)


def verify_index(config: CairnConfig) -> tuple[bool, tuple[str, ...]]:
    config.validate()
    with SQLiteIndex(config.resolve(config.index.database), readonly=True) as index:
        return index.verify()
