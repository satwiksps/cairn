...
from cairn_rag.content.hashing import hash_fields

EMPTY_MERKLE_ROOT = hash_fields("cairn-merkle-empty-v1", ())


def _leaf_hash(value: str) -> str:
    return hash_fields("cairn-merkle-leaf-v1", (value,))


def _parent_hash(left: str, right: str | None) -> str:
    return hash_fields("cairn-merkle-node-v1", (left, right or ""))
...
def corpus_root(document_roots: Mapping[str, str] | Sequence[tuple[str, str]]) -> str:
    ...
    leaves = (
        hash_fields("cairn-corpus-document-v1", (document_id, root))
