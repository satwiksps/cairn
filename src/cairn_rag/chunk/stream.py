...
NORMALIZER_VERSION = "cairn-normalizer-v1"
...
def default_token_count(word: str) -> int:
    """Return Cairn's model-independent v1 token estimate (one per word)."""
...
def resolve_tokenizer_id(
    ...
    ``word-v1`` is reserved for Cairn's built-in one-token-per-word counter.
    ...
    if tokenizer_id == DEFAULT_TOKENIZER_ID:
        raise ValueError(
            ...
            "the default; "
            f"{DEFAULT_TOKENIZER_ID!r} is reserved for Cairn's default token counter"
        )
    return tokenizer_id
