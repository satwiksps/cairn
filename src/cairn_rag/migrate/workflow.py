...
from cairn_rag.config import CairnConfig, load_config, loads_config, pending_migration_path
from cairn_rag.embed import embedding_identity
from cairn_rag.errors import BackendError, ConfigError
from cairn_rag.index.adapters import SQLiteIndex
from cairn_rag.index.service import ApplyResult, PreparedIndex, apply_prepared, prepare_index
from cairn_rag.migrate.planner import MigrationPlan
...
def _load_config_text(config_path: Path, text: str) -> CairnConfig:
    return loads_config(text, base_dir=config_path.parent, config_path=config_path)


def _config_changes(before: CairnConfig, after: CairnConfig) -> dict[str, dict[str, object]]:
...
class PreparedMigration:
    config_path: Path
    current_config: CairnConfig
    desired_config: CairnConfig
    before_config: str
    after_config: str
...
def _history_directory(config: CairnConfig) -> Path:
    database = config.resolve(config.index.database)
    return database.with_name(f"{database.name}.migrations")


def _latest_receipt(config: CairnConfig, config_path: Path) -> dict[str, Any]:
...
def recover_pending_migration(config_path: str | Path) -> RecoveryResult:
    ...
    if old_state:
        if current_sha != old_sha:
            raise ConfigError(
                "The index did not commit the migration, but cairn.toml changed while it was "
    ...
    if current_sha == old_sha:
        ...
        try:
            ...
            raise ConfigError(
                f"Index migration committed, but cairn.toml could not be finalized: {exc}. "
                "Rerun 'cairn-rag migrate --recover'."
            ) from exc
    elif current_sha != new_sha:
        raise ConfigError(
            "Index migration committed, but cairn.toml was edited concurrently. Preserve the "
...
def apply_migration(migration: PreparedMigration) -> MigrationResult:
    ...
    if _sha256(current_payload) != _sha256(before_payload):
        raise ConfigError("cairn.toml changed after the migration plan was prepared; re-plan")
