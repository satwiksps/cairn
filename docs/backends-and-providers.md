...
Placeholders are intentional: Cairn does not select a current model, dimensions, endpoint, or price on the operator's behalf. Omit `price_per_million_tokens` when it is unknown; otherwise set it from a current, verified source.
...
- `price_per_million_tokens` and `measure churn --price` are caller-supplied reporting inputs. Cairn does not fetch, date, or verify provider prices.
- Cairn's deterministic token estimate may differ from a provider's tokenizer and billing units. Treat the plan as an estimate, then validate it against the selected provider's current documentation and limits.
...
- The local SQLite index stores chunk text, metadata, and vectors; the cache stores vectors and identity metadata. Neither file is application-level encrypted by Cairn. Protect files with appropriate host access controls, encryption, backup, and retention policies.
